...
def eval_fitness(genomes, config):
    ...
    for j, (i, g) in enumerate(genomes):
        ...
        while True:
            ...
            #  if prev_dist == 0 and LSW.nearestDist() != 0 and instruction != 4:
                #  g.fitness -= 30

            #  if LSW.nearestDist() == 0:
                #  g.fitness += 20

            #  if LSW.nearestDist() == 0 
            #  elif LSW.nearestDist() == prev_dist and prev_dist != 0:
                #  score -= 500

            #  if LSW.nearestDist() == 0:
                #  if LSW.nearestDist() == prev_dist and instruction == 4:
                    #  score -=50
                #  else:
                    #  score += 30
...
if __name__ == "__main__":
    ...
    #  pop = neat.Checkpointer.restore_checkpoint('checkpoint/exp1/neat-checkpoint-1499')

    pop.add_reporter(neat.StdOutReporter(True))
    stats = neat.StatisticsReporter()
    pop.add_reporter(stats)
    pop.add_reporter(neat.Checkpointer(100))
    winner = pop.run(eval_fitness, 10)
