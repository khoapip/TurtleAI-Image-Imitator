...
class Turtle:
    ...
    def dot(self):
        if self.array[int(round(self.position[0]))][int(round(self.position[1]))]:
            self.array[int(round(self.position[0]))][int(round(self.position[1]))] = False
        else:
            self.__draw_line(self.__c, self.__r)
    ...
    def reset(self):
        ...
        """
        self.__r, self.__c, self.__dir__ = 0,0,0
        del self.__stack[:]
